...
public class MmoServerConfigTemp {
	...
	public ClientComponent getClient(String type, String name, String remoteIp, int remotePort) throws Exception {
		ClientConfig clientConfig = getClientConfig(type, name);
		ClientComponent client = new ClientComponent();
		client.setEventLoop(clientConfig.getWorkeThreadNum());
		client.setInProtoLength(clientConfig.getInProtoLength());
		client.setOutProtoLength(clientConfig.getOutProtoLength());
		client.setListener((ClientListener) Class.forName(clientConfig.getListener()).newInstance());
		client.setConnect(remoteIp, remotePort);
		return client;
	}

	public ServerComponent getServer(String type, String name, int port) throws Exception {
		ServerConfig serverConfig = getServerConfig(type, name);
		ServerComponent server = new ServerComponent();
		server.setEventLoop(serverConfig.getBoosThreadNum(), serverConfig.getWorkeThreadNum());
		server.setInProtoLength(serverConfig.getInProtoLength());
		server.setOutProtoLength(serverConfig.getOutProtoLength());
		server.setListener((ServerListener) Class.forName(serverConfig.getListener()).newInstance());
		server.setPort(port);
		return server;
	}

	public MysqlComponent getMysql(String type, String name, String hikariconfig) throws Exception {
		MysqlConfig mysqlConfig = getMysqlConfig(type, name);
		MysqlComponent mysql = new MysqlComponent();
		mysql.setHikaricpConfigPaht(hikariconfig);
		mysql.setListener((MysqlListener) Class.forName(mysqlConfig.getListener()).newInstance());
		mysql.setUseThread(mysqlConfig.getWorkeThreadNum());
		return mysql;
	}
